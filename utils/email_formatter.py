...
class EmailContentProcessor:
    ...
    def extract_clean_text_from_html(html_content: str) -> str:
        ...
        try:
            ...
            for blockquote in soup.find_all(['blockquote', 'div']):
                ...
                if blockquote and hasattr(blockquote, 'get') and blockquote.get('class'):
                    classes = ' '.join(blockquote.get('class', []))
                    if any(keyword in classes.lower() for keyword in ['quote', 'gmail', 'outlook']):
    ...
    def clean_email_text(text: str) -> str:
        ...
        if clean_text:
            ...
            for line in lines:
                ...
                is_signature = any(re.match(pattern, line_stripped, re.IGNORECASE) 
    ...
    @staticmethod
    def format_thread_message(subject: str, sender: str, recipient: str, 
                            date: str, html_content: str, text_content: str) -> dict:
        ...
        if html_content:
            clean_text = EmailContentProcessor.extract_clean_text_from_html(html_content)
        ...
        if not clean_text:
            if text_content:
                clean_text = text_content[:1000] + "..." if len(text_content) > 1000 else text_content
            elif html_content:
                soup = BeautifulSoup(html_content, 'html.parser')
                clean_text = soup.get_text()[:1000] + "..." if len(soup.get_text()) > 1000 else soup.get_text()
...
def format_email_for_display(html_content: str, text_content: str, 
                           max_length: int = 500) -> Tuple[str, str]:
    """
    ...
    try:
        ...
        if text_content:
            safe_text = text_content[:max_length] + "..." if len(text_content) > max_length else text_content
            return safe_text, "fallback_text"
        elif html_content:
            try:
                soup = BeautifulSoup(html_content, 'html.parser')
                basic_text = soup.get_text() if soup else ""
                safe_text = basic_text[:max_length] + "..." if len(basic_text) > max_length else basic_text
...
def extract_email_preview(html_content: str, text_content: str) -> str:
    ...
    clean_content, _ = format_email_for_display(html_content, text_content, max_length=200)
